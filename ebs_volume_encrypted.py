...
def get_session(role_arn=None):
    if role_arn:
        ...
        assumed = sts.assume_role(
            RoleArn=role_arn,
            RoleSessionName="ebs-volume-encryption-audit"
        )
...
def check_ebs_volume_encryption(session):
    ...
    for region in tqdm(regions, desc="Scanning Regions"):
        ...
        try:
            ...
            if error_code in ["AuthFailure", "OptInRequired"]:
                print(f"Skipping region {region} during volume scan (not accessible)")
                continue
            else:
...
def write_csv(account_id, results):
    filename = f"ebs_volume_encryption_{account_id}.csv"
...
def main():
    ...
    results, total_checked, compliant, non_compliant = \
        check_ebs_volume_encryption(session)

    print("\n====================================================")
    print("CONTROL: EBS Volumes Are Encrypted")
