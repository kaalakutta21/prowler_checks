...
def get_session(role_arn=None):
    if role_arn:
        ...
        assumed = sts.assume_role(
            RoleArn=role_arn,
            RoleSessionName="ec2-ssm-managed-audit"
        )
...
def check_ec2_ssm(session):
    ...
    for region in tqdm(regions, desc="Scanning Regions"):
        ...
        try:
            for page in ssm_paginator.paginate():
                for inst in page.get("InstanceInformationList", []):
                    ssm_instances.add(inst["InstanceId"])

        except ClientError as e:
            code = e.response["Error"]["Code"]

            if code in ["AccessDeniedException", "UnauthorizedOperation"]:
                print(f"Skipping SSM in region {region} (SCP Denied)")
                skipped_regions += 1
                continue
            else:
                continue
        ...
        try:
            for page in ec2_paginator.paginate():
                ...
                for res in page.get("Reservations", []):
                    for inst in res.get("Instances", []):
                        ...
                        if instance_id in ssm_instances:
                            status = "COMPLIANT"
                            reason = "Managed by SSM"
                        else:
            ...
            if code in ["AccessDeniedException", "UnauthorizedOperation"]:
                print(f"Skipping EC2 in region {region} (SCP Denied)")
                skipped_regions += 1
                continue
            else:
                continue
...
def write_csv(account_id, results):
    ...
    filename = f"ec2_ssm_managed_{account_id}.csv"
...
def main():
    ...
    results, total, compliant, non_compliant, skipped_regions = \
        check_ec2_ssm(session)

    print("\n====================================================")
    print("CONTROL: EC2 Managed by SSM")
