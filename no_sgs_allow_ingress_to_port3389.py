...
def is_rdp_exposed(permission):
    ...
    if protocol == "-1":
        ...
        if from_port is None or to_port is None:
            return False
        port_match = from_port <= 3389 <= to_port
...
def check_rdp_exposure(session):
...
def write_csv(account_id, results):
    filename = f"sg_rdp_exposure_{account_id}.csv"
...
def main():
    parser = argparse.ArgumentParser(
        description="Ensure no Security Groups allow RDP from 0.0.0.0/0 or ::/0"
    )
    ...
    results, total_checked, compliant, non_compliant = \
        check_rdp_exposure(session)

    print("\n====================================================")
    print("CONTROL: No Internet RDP Exposure (Port 3389)")
